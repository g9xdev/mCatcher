...
> **STATUS: implemented, then reverted. Do not re-implement as written.** Landed as `096aca9`, reverted by `3ce4449`. The invariant work was sound; the deadline itself is not. In production the scheduler clock is real — `background.js` drives `tick(Date.now())` every second — so a fixed 120s bound is an always-armed wall-clock timer over a drain whose honest duration is unbounded: an assembled job in `pausing_provider` holds its assembly-sink lease across in-flight segment fetches (no timeout anywhere in `background-adapters.js`) and across the whole native sink commit, minutes for a large video. The pause is triggered by provider saturation (429/5xx/stall), the very condition that makes those fetches slow. On a misfire `enterNeedsUser` runs, the real terminal that eventually arrives is dropped by the `needs_user` guard, and a completed download is silently discarded — worse than the latent wedge it removes. The task text below is left as written for the record; anything replacing it needs a bound derived from observed progress, not from wall-clock time.

...
test("a pausing_provider drain that never quiesces gives up its slot", () => {
  // pausing_provider is the only non-running state that may hold a slot, and the
  // only exit is quiescence. A lost owner terminal never releases the permit, so
  // without a deadline this job holds the slot forever and starves its siblings.
  let now = 0;
  ...
  // A tick well inside the deadline must not disturb a legitimately draining job.
  now = 60000;
  s.tick(now);
  assert.equal(s.getJob("sib").state, "pausing_provider");
  assert.equal(s.getJob("sib").holdsGlobalSlot, true);
